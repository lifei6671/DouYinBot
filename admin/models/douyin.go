...
type DouYinVideo struct {
	...
	Nickname         string    `orm:"column(nickname);size(100); not null;description(作者昵称)"`
	...
	VideoRawPlayAddr string    `orm:"column(video_raw_play_addr);not null;size(2000);description(原视频地址)"`
	VideoPlayAddr    string    `orm:"column(video_play_addr);size(2000);not null;description(视频原播放地址)"`
	VideoId          string    `orm:"column(video_id);size(255);unique;not null;description(视频唯一ID)"`
	VideoCover       string    `orm:"column(video_cover);size(2000);null;description(视频封面)"`
	VideoLocalAddr   string    `orm:"column(video_local_addr);size(2000);not null;description(本地路径)"`
