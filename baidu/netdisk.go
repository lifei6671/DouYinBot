...
func (d *NetDisk) UploadFiles(uploadFile *PreCreateUploadFile, reader io.Reader) ([]SuperFile, error) {
	...
	for i := 0; ; i++ {
		...
		if superFile.ErrorNo != 0 {
			return nil, fmt.Errorf("分片上传文件失败 -> [resp:%s] [errcode:%d] [errormsg:%s]", string(respBody), superFile.ErrorNo, superFile.ErrorMsg)
		}
		d.printf("分片上传成功->[index:%d]  [size:%d] [resp:%s]", i, n, string(respBody))
...
func (d *NetDisk) CreateFile(uploadFile *CreateFileParam) (*CreateFile, error) {
	...
	if createFile.ErrNo != 0 {
		return nil, fmt.Errorf("创建文件失败 -> [errno=%d]", createFile.ErrNo)
	}
